...
DS = ['mnist','cifar10','cifar100','svhn2']
...
for ds in DS:
	...
	dataset  = load(ds)
	settings = '-d {} -n {} -e %s -b 128 -lrnalg sgd -lrnparam 1e-3 1e-3 0.9 -k -v 2' % (100 if ds != 'svhn2' else 10)

	for bd1, bd2, bd3, bd4, bd5, m in product([1,2,3], [1,2,3], [1,2,3], [1,2,3], [1,2], [0,1]):

		par = [str(p) for p in [ds, bd1, bd2, bd3, bd4, bd5, m]]
		par = '-'.join(par)
